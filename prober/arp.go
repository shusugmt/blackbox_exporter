...
import (
	...
	. "github.com/prometheus/blackbox_exporter/prober/arp"
)
...
func ProbeARP(ctx context.Context, target string, module config.Module, registry *prometheus.Registry, _ log.Logger) (success bool) {
	...
	// 既にパース済みであれば、そこからパラメータを取ってくる
	pk := ParamKey{
		Interface:    srcIntfName,
		TargetString: target,
	}
	var p Parameter
	param, ok := ParamMap.Load(pk)
	if ok {
		p = param.(Parameter)
	} else {
		level.Info(logger).Log("msg", "Parameter not found. Building for first time")
		p = Parameter{}
	...
	logger = log.With(logger, "interface", p.Intf.Name)
	for _, t := range p.Tags {
		logger = log.With(logger, t.K, t.V)
	}

	ethHdr, arpRequestHdr, arpResponseHdr := buildARPRequest(p.SrcMAC, p.DstMAC, p.SrcMAC, p.SPA, p.TPA)

	// Open up a pcap handle for packet reads/writes.
	handle, err := pcap.OpenLive(p.Intf.Name, 128, false, pcap.BlockForever)
	if err != nil {
		level.Error(logger).Log("msg", err)
		return false
	}
	...
	bpfFilter := fmt.Sprintf("ether src %s and arp and arp[6:2] = 2", p.DstMAC.String())
	...
	receiver, err := OpenARPReceiver(p.Intf)
	if err != nil {
		level.Error(logger).Log("msg", err)
		return false
	}
	defer receiver.Close()
	go readARP(ctx, receiver, arpResponseHdr, &responseReceived, wg)
...
func parseTargetString(targetString string) (params targetParams, err error) {
	...
	if len(p) == 3 {
		...
		for _, tagkv := range strings.Split(t, ",") {
			...
			params.addTag(Tag{K: kv[0], V: kv[1]})
		}
	}
	return params, nil
}
...
func readARP(ctx context.Context, r *ARPReceiver, arpResponseHdr *layers.ARP, responseReceived *bool, wg *sync.WaitGroup) {
	defer wg.Done()
